...
describe("summoner-web server", { concurrency: 1 }, () => {
  ...
  test("GET / with token → 200 HTML workbench", async () => {
    ...
    assert.match(r.body, /function showStreamMsg/)
    assert.match(r.body, /function clearStreamMsg/)
    assert.match(r.body, /var streamMsg=null/)
    assert.match(r.body, /msg assistant streaming/)
    assert.match(r.body, /\.msg\.assistant\.streaming::after\{[^}]*animation:streamBlink/)
