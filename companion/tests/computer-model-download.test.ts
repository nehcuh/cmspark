...
// 覆盖：断点续传拼接、分片篡改最终哈希检出、原子 rename 不留半成品、预算（models/
// 根全局核算，M3）/卷满下载前拒下、超限流截流（M1：Content-Length 预检 + 流内 abort）、
// 幂等跳过（M5：在盘哈希命中零 fetch）、file:// 镜像拒绝、失败审计事件形状、
// stale .part 四态清理（超期/revision 变更/url 变更/meta 孤儿）、Range 200 回退重写、
// 416 删片重下、删除模型。
...
function makeEnv() {
  ...
  // 嵌套目标目录（models/tinyclick-hybrid）：预算按 destDir 父目录全局核算（M3），
  // 测试需要隔离的 models/ 根——直接以 mkdtemp 根当 destDir 会让预算核算落到系统 tmp；
  // 预建目录供各用例的种子文件（.part/meta/在盘模型）直接写入
  const destDir = path.join(dir, "models", "tinyclick-hybrid")
...
test("download: 全新下载成功——文件齐全、无 .part 残留、totalBytes 正确", async () => {
  ...
  try {
    ...
    const result = await downloadModelVariant(
      { manifest, modelId: "tinyclick", variant: "hybrid", destDir: env.destDir },
      { ...env.deps, fetchImpl: makeFakeFetch(routes) },
    )
    assert.deepStrictEqual(readFileSync(path.join(env.destDir, "a.onnx")), FILE_A)
    assert.deepStrictEqual(readFileSync(path.join(env.destDir, "b.onnx")), FILE_B)
    ...
    // 无 .part / meta 残留
    const left = readdirSync(env.destDir).filter((n) => n.includes(".part"))
...
test("download: 断点续传拼接——第二请求带 Range，最终文件哈希通过", async () => {
  ...
  try {
    ...
    seedPart(env.destDir, "a.onnx", FILE_A)
    ...
    assert.deepStrictEqual(readFileSync(path.join(env.destDir, "a.onnx")), FILE_A)
    assert.strictEqual(existsSync(path.join(env.destDir, "a.onnx.part")), false)
    assert.strictEqual(existsSync(path.join(env.destDir, "a.onnx.part.json")), false)
  } finally {
    env.cleanup()
  }
})
...
test("download: 分片篡改——最终全量哈希复验检出并删除分片", async () => {
  ...
  try {
    ...
    assert.strictEqual(existsSync(path.join(env.destDir, "a.onnx.part")), false)
    assert.strictEqual(existsSync(path.join(env.destDir, "a.onnx.part.json")), false)
    assert.strictEqual(existsSync(path.join(env.destDir, "a.onnx")), false)
    // 审计事件形状
...
test("download: 原子 rename——下载中途崩溃不留半成品最终文件", async () => {
  ...
  try {
    ...
    assert.strictEqual(existsSync(path.join(env.destDir, "a.onnx")), false) // 无半成品最终文件
    assert.strictEqual(existsSync(path.join(env.destDir, "a.onnx.part")), true) // 分片残留供续传
  } finally {
    env.cleanup()
  }
})
...
test("download: 磁盘预算超限——下载前拒下，fetch 零调用", async () => {
  ...
  try {
    ...
    await assert.rejects(
      () =>
        downloadModelVariant(
          { manifest, modelId: "tinyclick", variant: "hybrid", destDir: env.destDir, diskBudgetMB: 0.001 },
          { ...env.deps, fetchImpl },
        ),
...
test("download: file:// 镜像拒绝（mirror-scheme-denied + 审计）", async () => {
  ...
  try {
    ...
    await assert.rejects(
      () =>
        downloadModelVariant(
          { manifest, modelId: "tinyclick", variant: "hybrid", destDir: env.destDir, mirror: "file:///C:/evil" },
...
test("oversize: Content-Length 预检——声明字节超申报，零写盘拒绝", async () => {
  ...
  try {
    ...
    assert.strictEqual(existsSync(path.join(env.destDir, "a.onnx.part")), false) // 零写盘
    assert.strictEqual(env.events[0]!.payload.reason, "oversize-stream")
  } finally {
    env.cleanup()
  }
...
test("stale: revision 变更 → 删旧分片从头重下（无 Range）", async () => {
  ...
  try {
    const manifest = makeManifest([{ name: "a.onnx", content: FILE_A }])
    const url = manifest.models.tinyclick!.variants.hybrid!.files[0]!.url
    seedPart(env.destDir, "a.onnx", FILE_A, { revision: "aaaaaaaabbbbbbbbccccccccddddddddeeeeeeee" })
    const route: FakeRoute = { body: FILE_A, seenRanges: [] }
    await downloadModelVariant(
      { manifest, modelId: "tinyclick", variant: "hybrid", destDir: env.destDir },
      { ...env.deps, fetchImpl: makeFakeFetch({ [url]: route }) },
...
test("stale: 超期（>PART_STALE_MS）→ 删旧分片从头重下", async () => {
  ...
  try {
    ...
    seedPart(env.destDir, "a.onnx", FILE_A, { startedAt: t0 - PART_STALE_MS - 1 })
    const route: FakeRoute = { body: FILE_A, seenRanges: [] }
    await downloadModelVariant(
      { manifest, modelId: "tinyclick", variant: "hybrid", destDir: env.destDir },
...
test("stale: meta 缺失（孤儿 .part）→ 删了从头重下", async () => {
  ...
  try {
    const manifest = makeManifest([{ name: "a.onnx", content: FILE_A }])
    const url = manifest.models.tinyclick!.variants.hybrid!.files[0]!.url
    writeFileSync(path.join(env.destDir, "a.onnx.part"), FILE_A.subarray(0, 1000)) // 无 meta
    const route: FakeRoute = { body: FILE_A, seenRanges: [] }
    await downloadModelVariant(
      { manifest, modelId: "tinyclick", variant: "hybrid", destDir: env.destDir },
      { ...env.deps, fetchImpl: makeFakeFetch({ [url]: route }) },
...
test("stale: meta.url 与解析 url 不符（镜像变更）→ 从头重下", async () => {
  ...
  try {
    const manifest = makeManifest([{ name: "a.onnx", content: FILE_A }])
    const url = manifest.models.tinyclick!.variants.hybrid!.files[0]!.url
    seedPart(env.destDir, "a.onnx", FILE_A, { url: "https://old-mirror.example/x/a.onnx" })
    const route: FakeRoute = { body: FILE_A, seenRanges: [] }
    await downloadModelVariant(
      { manifest, modelId: "tinyclick", variant: "hybrid", destDir: env.destDir },
      { ...env.deps, fetchImpl: makeFakeFetch({ [url]: route }) },
...
test("range: 服务端对 Range 返回 200（不支持续传）→ 从头重写，文件完整", async () => {
  ...
  try {
    ...
    assert.deepStrictEqual(readFileSync(path.join(env.destDir, "a.onnx")), FILE_A) // 从头重写后完整
  } finally {
    env.cleanup()
  }
})
...
test("range: 分片已齐未 rename（上次崩在复验前）→ 零请求直接复验 rename", async () => {
  ...
  try {
    ...
    const url = manifest.models.tinyclick!.variants.hybrid!.files[0]!.url
    // 分片完整 + meta 有效
    writeFileSync(path.join(env.destDir, "a.onnx.part"), FILE_A)
    const f = manifest.models.tinyclick!.variants.hybrid!.files[0]!
    writeFileSync(
      path.join(env.destDir, "a.onnx.part.json"),
...
test("idempotent: 全变体已在盘且哈希命中 → 零 fetch 跳过，残留分片顺手清理", async () => {
  ...
  try {
    ...
    writeFileSync(path.join(env.destDir, "a.onnx"), FILE_A)
    writeFileSync(path.join(env.destDir, "b.onnx"), FILE_B)
    writeFileSync(path.join(env.destDir, "b.onnx.part"), FILE_B.subarray(0, 100))
    writeFileSync(path.join(env.destDir, "b.onnx.part.json"), "{}")
    const routes = env.routeFor(manifest)
    const result = await downloadModelVariant(
      { manifest, modelId: "tinyclick", variant: "hybrid", destDir: env.destDir },
      { ...env.deps, fetchImpl: makeFakeFetch(routes) },
    ...
    assert.deepStrictEqual(result.files, ["a.onnx", "b.onnx"])
    assert.strictEqual(existsSync(path.join(env.destDir, "b.onnx.part")), false)
    assert.strictEqual(existsSync(path.join(env.destDir, "b.onnx.part.json")), false)
...
test("idempotent: 在盘文件哈希不符（本地被改）→ 正常重下并替换", async () => {
  ...
  try {
    ...
    writeFileSync(path.join(env.destDir, "a.onnx"), tampered)
    const route: FakeRoute = { body: FILE_A, seenRanges: [] }
    await downloadModelVariant(
      { manifest, modelId: "tinyclick", variant: "hybrid", destDir: env.destDir },
      { ...env.deps, fetchImpl: makeFakeFetch({ [url]: route }) },
    ...
    assert.deepStrictEqual(readFileSync(path.join(env.destDir, "a.onnx")), FILE_A) // 被替换为正确字节
  } finally {
    env.cleanup()
  }
})
...
test("delete: 删除模型目录回收全部字节（含 .part/meta）；目录不存在返回 0", async () => {
  ...
  try {
    ...
    writeFileSync(path.join(env.destDir, "b.onnx.part"), FILE_B.subarray(0, 100))
    writeFileSync(path.join(env.destDir, "b.onnx.part.json"), "{}")
    const result = await deleteModelVariant({ variant: "hybrid", destDir: env.destDir })
    assert.strictEqual(result.removedBytes, FILE_A.byteLength + 100 + 2)
    assert.strictEqual(existsSync(env.destDir), false)
    const again = await deleteModelVariant({ variant: "hybrid", destDir: env.destDir })
...
test("budget: 预算按 models/ 根全局核算（M3）——兄弟变体占用计入", async () => {
  const env = makeEnv()
  try {
    const manifest = makeManifest([{ name: "a.onnx", content: FILE_A }]) // 本次 2048B
    const sibling = path.join(env.dir, "models", "tinyclick-int8")
    mkdirSync(sibling, { recursive: true })
    writeFileSync(path.join(sibling, "v.onnx"), FILE_B) // 兄弟变体占用 1024B
    // 预算 = 兄弟占用 + 本次总量 - 1 字节 → 全局核算下必超
    const budgetMB = (FILE_B.byteLength + FILE_A.byteLength - 1) / (1024 * 1024)
    await assert.rejects(
      () =>
        downloadModelVariant(
          { manifest, modelId: "tinyclick", variant: "hybrid", destDir: env.destDir, diskBudgetMB: budgetMB },
          { ...env.deps, fetchImpl: makeFakeFetch(env.routeFor(manifest)) },
        ),
      (e) => (expectDownloadError(e, "disk-budget-exceeded"), true),
    )
    // 对照：同预算按 per-variant 核算（budgetDir=destDir，M3 前语义）则放行——证明核算范围差异
    const ok = await downloadModelVariant(
      { manifest, modelId: "tinyclick", variant: "hybrid", destDir: env.destDir, diskBudgetMB: budgetMB, budgetDir: env.destDir },
      { ...env.deps, fetchImpl: makeFakeFetch(env.routeFor(manifest)) },
    )
    assert.strictEqual(ok.files.length, 1)
  } finally {
    env.cleanup()
  }
})
