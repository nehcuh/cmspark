...
// --- P3：下载进度广播节流（I4 对抗；整百分点前进 或 ≥200ms） -------------------------

test("P3：per-chunk 进度回调按整百分点节流——1000 次回调仅 101 条广播，首帧 0% 与末帧 100% 必达", async () => {
  resetModelConfig({ coordinateEnabled: false, modelMirror: "https://hf-mirror.example", modelVariant: "int8" })
  const broadcasts: any[] = []
  const downloadImpl = async (_args: any, opts: any) => {
    for (let i = 1; i <= 1000; i++) opts.onProgress("a.onnx", i * 100, 100000) // 0.1%/次 × 10 次/百分点
    return {} as any
  }
  const r = await handleComputerModelMessage(
    { type: "computer.model.download", source: "settings" },
    { broadcast: (d: any) => broadcasts.push(d) },
    holderWith(null),
    { manifestLoader: async () => realHostManifest(), downloadImpl: downloadImpl as any, now: () => 1000 },
  )
  assert.equal(r.status, "started")
  await flush(); await flush(); await flush()
  const progress = broadcasts.filter((b) => b.type === "computer.model.progress")
  assert.equal(progress.length, 101, "冻结时钟下仅百分点前进触发（0..100；万级 → 百级）")
  assert.equal(progress[0].receivedBytes, 100, "首帧 0% 必达")
  assert.equal(progress[progress.length - 1].receivedBytes, 100000, "末帧 100% 必达")
  const pcts = progress.map((b) => Math.floor((b.receivedBytes / b.totalBytes) * 100))
  for (let i = 0; i <= 100; i++) assert.equal(pcts[i], i, `第 ${i} 条广播应为 ${i}%（单调无重复）`)
})

test("P3：同百分点刷屏抑制 + ≥200ms 时间轴兜底", async () => {
  resetModelConfig({ coordinateEnabled: false, modelMirror: "https://hf-mirror.example", modelVariant: "int8" })
  const broadcasts: any[] = []
  let clock = 1000
  const downloadImpl = async (_args: any, opts: any) => {
    for (let i = 0; i < 500; i++) opts.onProgress("a.onnx", 500, 100000) // 同 pct 刷屏
    clock = 1300 // +300ms：时间轴兜底应放行一条
    opts.onProgress("a.onnx", 500, 100000)
    clock = 1350 // +50ms：<200ms 且 pct 未变 → 抑制
    opts.onProgress("a.onnx", 500, 100000)
    return {} as any
  }
  await handleComputerModelMessage(
    ...
    { broadcast: (d: any) => broadcasts.push(d) },
    holderWith(null),
    { manifestLoader: async () => realHostManifest(), downloadImpl: downloadImpl as any, now: () => clock },
  )
  await flush(); await flush(); await flush()
  const progress = broadcasts.filter((b) => b.type === "computer.model.progress")
  assert.equal(progress.length, 2, "首帧 + 时间轴兜底各一条；同 pct 且 <200ms 全抑制")
})
