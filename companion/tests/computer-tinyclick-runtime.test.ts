...
test("熔断: 3 次推理故障 → disabled + 审计 + 广播形状；手动复位后恢复", async () => {
  const h = makeHarness();
  await h.runtime.prepare();
  // 三次独立 worker 故障（每次故障后懒重建出新 worker 再崩）；首个 worker 追加
  // exit 尾随事件锁 M4：error+exit 序列只计一次
  for (let i = 0; i < 3; i++) {
    const w = h.workers[h.workers.length - 1]!;
    w.emit("error", new Error(`crash-${i}`));
    if (i === 0) w.emit("exit", 1); // M4：崩溃的 error+exit 序列不得双计
    assert.strictEqual(h.runtime.getFaults(), i + 1, `第 ${i + 1} 次故障应恰计 ${i + 1}`);
    if (i < 2) {
      await h.runtime.infer(FRAME, INPUT_IDS); // 触发懒重建（新 worker，fake 正常应答）
      assert.strictEqual(h.runtime.getStatus(), "warm");
    }
  }
...
// --- M3/M4（I2 对抗 P3-a/P3-b） -----------------------------------------------------

test("M3: warming 中 dispose → 无 worker 泄漏 + 后续调用全拒 model-disabled", async () => {
  const h = makeHarness();
  // load 延迟 60ms，制造 warming 窗口
  h.next().onMessage = (msg, reply) => {
    if (msg.type === "load") setTimeout(() => reply(loadedReply(msg.id ?? 0)), 60);
    else if (msg.type === "infer") reply(resultReply(msg.id ?? 0));
  };
  const p = h.runtime.prepare();
  await waitFor(() => h.workers.length >= 1, "worker 已 spawn");
  await h.runtime.dispose(); // warming 窗口内 dispose
  await expectCode(p, "model-disabled"); // prepare 落地前查 disposed → 拒
  await waitFor(() => h.workers[0]!.terminated >= 1, "落地 worker 被收尸");
  assert.strictEqual(h.runtime.getStatus(), "idle");
  assert.strictEqual(h.runtime.getFaults(), 0, "dispose 路径不计熔断");
  // 后续调用全拒
  await expectCode(h.runtime.infer(FRAME, INPUT_IDS), "model-disabled");
  await expectCode(h.runtime.prepare(), "model-disabled");
});

test("M4: 超时 terminate 后尾随 error 事件不双计熔断", async () => {
  const h = makeHarness({ inferTimeoutMs: 50 });
  hangRealInfer(h.next());
  await expectCode(h.runtime.infer(FRAME, INPUT_IDS), "infer-timeout");
  assert.strictEqual(h.runtime.getFaults(), 1);
  const w0 = h.workers[0]!;
  assert.strictEqual(w0.terminated, 1);
  // 撕毁中的 worker 尾随 error（真实 node Worker  terminate 期间可发）——不得再计
  w0.emit("error", new Error("trailing error during teardown"));
