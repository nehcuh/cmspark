...
/** #430: 内容风控隔离——只替换「最近的大型工具结果」（最可能的触发源）的长度门槛。 */
const CONTENT_RISK_QUARANTINE_MIN_LEN = 500
/** #430: 占位文本（诚实声明移除事实；不进 untrusted 包装——这是系统动作不是网页数据）。 */
const CONTENT_RISK_QUARANTINE_PLACEHOLDER =
  "[该段内容已被移除：触发服务商内容风控（400）。请基于其余信息继续，不要重复获取同一来源。]"

/**
 * #430: 从后往前找最近一条大型工具结果（内容风控的最可能触发源，如网页正文）。
 * 找不到（没有可隔离对象）→ 返回 -1，调用方直接致命，不做无谓重发。
 */
export function findLastLargeToolResultIndex(
  messages: Array<{ role?: string; content?: unknown }>,
): number {
  for (let i = messages.length - 1; i >= 0; i--) {
    const m = messages[i]
    if (m?.role === "tool" && typeof m.content === "string" && m.content.length >= CONTENT_RISK_QUARANTINE_MIN_LEN) {
      return i
    }
  }
  return -1
...
export function quarantinePersistedToolRow(
  ...
  if (!target) {
    for (let i = rows.length - 1; i >= 0; i--) {
      ...
      if (
        ...
        r.content.length >= CONTENT_RISK_QUARANTINE_MIN_LEN
      ) {
        target = r
        break
      }
  ...
  const toolCalls = Array.isArray(target.tool_calls)
    ? target.tool_calls.map((tc: any) => ({ ...tc, result: { quarantined: true, reason: "content_risk" } }))
    : target.tool_calls
  threadManager.updateMessage(threadId, target.id, {
...
export async function chatCreate(params: ChatCreateParams) {
  ...
  const hostUseRule12 = hostPlat === "win32"
   ...
   NEVER default to host_computer for browser-DOM — use get_page_text / click({text}) / type / evaluate first. After CDP attach freeze / DOM-script cap, OR when the user explicitly asks for 模拟点击/像素点击, you MAY call host_computer on the Chrome app token. That ALWAYS pops a confirm (无人值守/三旗 will NOT skip it). When escalating from a CDP failure, pass trigger_reason (e.g. "CDP 定位失败 → 坐标化升级") — it is shown in the confirm. On browsers locate uses UIA+OCR only (no VLM); a missing anchor fails honestly — report it instead of retrying blindly. Password managers / terminals / LOLBIN remain impossible.
   ...
   - host_read: read top-1 Mail inbox. Returns {sender, subject, date_received, body_preview, verified, summary}. max_chars (1-5000, default 500) is honored end-to-end — pass a larger value when the user needs more of the body. Only claim you "read the mail" when verified===true.
   ...
   NEVER default to host_computer for browser-DOM — use get_page_text / click({text}) / type / evaluate first. After CDP attach freeze / DOM-script cap, OR when the user explicitly asks for 模拟点击/像素点击, you MAY call host_computer on the Chrome app token. That ALWAYS pops a confirm (无人值守/三旗 will NOT skip it). When escalating from a CDP failure, pass trigger_reason (e.g. "CDP 定位失败 → 坐标化升级") — it is shown in the confirm. On browsers locate uses UIA+OCR only (no VLM); a missing anchor fails honestly — report it instead of retrying blindly. Password managers / terminals / LOLBIN remain impossible. NEVER propose speculatively.`
  ...
  while (round < MAX_TOOL_CALL_ROUNDS) {
    ...
    try {
      ...
      if (isContentRisk) {
        ...
        logger.error("llm.content_risk_fatal", { error: errorMsg, round })
        sendToExtension({
          type: "chat.error",
          thread_id: threadId,
          error: "服务商内容风控拦截了本次请求，已停止。可尝试：新开对话、调整任务描述，或在设置中更换模型。",
        })
        if (runStats) runStats.terminal = "error"
        return
      }
