...
func runReadMessage(targetId: String) throws -> String {
    ...
    on jsonEscape(s)
        ...
        -- C0 whitelist pass: any remaining control char (< 32, and not one of
        -- the JSON-legal escapes 8/9/10/12/13 handled above) becomes its \\u00XX
        -- escape. Without this a body containing e.g. \\x07 (BEL) or \\x0B (VT)
        -- would emit invalid JSON (TS parseJsonSafe fails closed and the mail
        -- becomes unreadable). Issue #69 F1.
...
/// HID requires Accessibility for CMspark; activate first so frontmost
...
func cuInject(action: String, windowId: UInt32, x: Int?, y: Int?, text: String?, chord: String?, delta: Int?, checkOcclusion: Bool, checkSecureInput: Bool, checkOnscreen: Bool, estopFlag: String?) -> String {
    ...
    if useHid && !AXIsProcessTrusted() {
        fputs("[host] WARNING: CMspark is not Accessibility-trusted; " +
              "HID inject may be dropped. Grant System Settings → Privacy & Security → Accessibility.\n", stderr)
    }
