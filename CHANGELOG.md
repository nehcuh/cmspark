...
- 可选 MCP 上下文出口（#456）：独立 `outbound_context_v1` 档案按精确 origin、所选知识和具体 grant/session 导出上下文；页面许可保持独立，异步返回前复核撤销与文档版本，失效会话不自动重试操作。提供设置/CLI 入口，default/interact 工具集合保持原有范围。
