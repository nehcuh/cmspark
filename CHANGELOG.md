...
- 操作面多路对抗复审修复（#504–#511）：同进程续跑/「继续」不再受归档桩影响，第二段保留第一段工具输出的完整上下文（仅进程内存，磁盘仍只存指纹）；未挂循环的对话触顶 100 步后显示可继续提示，不再静默截断；归档失败步骤同样省略填充值与页面正文，只保留错误原因与少量机器字段；Mode C 内嵌终端启动后状态如实（运行中横幅、启动入口收起、停止语义诚实），启动失败不吞已确认意图；待确认的 L2 工具步骤只影响当前对话当前轮，不再跨线程/跨回合误标已完成步骤。
