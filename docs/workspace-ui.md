...
- **最近对话**：按最后消息时间排列，可以筛选标题。子任务默认不摊成平级聊天；主任务行显示「N 子任务」。点击最近对话旁“管理对话”或顶栏“对话管理”，进入完整检索、分组、回收站和批量操作。
...
“整理助手”是规则扫描，供核对后提取要点或移入回收站（提供全选/全不选；删除确认条钉在面板顶部，不走会被侧栏吞掉的系统对话框）。对话管理工具栏有「全选」当前列表可删会话。行内删除同样在面板内确认后移入回收站。回收站列出全部已删行（含子任务），提供「清空回收站」（确认后硬删；约 30 天未清的仍会自动过期）。“关系图谱”打开原有会话关系图。管理面板不会阻挡运行中的停止按钮，待确认操作到达时会自动收起。分类保存需收到服务端确认；如果旧 Companion 未支持人工标签，会提示更新，不会显示假成功。
