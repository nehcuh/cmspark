...
> 版本: 2.4.9 | 日期: 2026-09-20 | 状态: 已确认（同步 **0.6.8** + **[ADR-020](adr/020-capability-model-three-axes.md) 能力三轴** · 知识库波次（AI 草稿 / 检索 / 分布 / 文件夹）· context_window 512000 · listen-first · 当轮活计划 · ADR-023/024 语音（含 2026-08-31 L13 非静默回退）· Capture HTML 卡默认 1040×760（紧凑 360×420）流式 · 用户附图 · Windows NSIS · 知识诚实（TF-IDF top-k）· 知识图谱 #427 · Darwin 内嵌终端 #432（默认关）· `search_threads`/`search_knowledge` #439 · ChatShell · 0.6.0 自主性三件套（巡航档位 / plan_readonly / loop L-1–L-5）· 专家团队 v1 · CU 完整性链（后三项索引级，详见 [CHANGELOG](../CHANGELOG.md) `[0.6.0]`，loop/专家独立 ADR 待补；CU 定位仍实验，#363 未过）；T1 **已记分、禁扩 profile**）
...
| Computer 坐标 | `host_computer` · `computer.coordinateEnabled` · `coordinateAllowed` | 双开关 + 任务级 L2：**god-mode / auto_approve 永不跳过**；**浏览器**走一次性 L2（值守/三旗/G1 永不 skip）；session-trust 可抑 mid-task re-L2，且（显式 opt-in + corpus/预算/actions/thread key）可跳过同线程同 **非浏览器** App 后续任务的 **initial L2**；danger / experimental / foreground_yielded **始终 prompt** |
