...
> **Current direction: [root DESIGN.md](../DESIGN.md), #469 / #471, 2026-09-08.**
...
| Surface | Design job |
|---------|------------|
| 召唤器 | Mac Swift = collapsed bar + **展开对话** (旧条; 本票不翻). HTML Capture 卡片 **360×420** (inner `OVERLAY_WINDOW_SIZE`; 流式 `chat.token`): 无装饰标记, **要我帮你做什么**, 📎 🎙, **开始会议**, **打开浏览器并打开侧栏**. HTML 听写不再「听写在侧栏」. **No** `当前页` on overlay. F-I-4: Companion 不调 `chrome.*`; 失败 toast **请点工具栏 C**; extension SW 开侧栏. **贴回侧栏** still not a filled button. Same neutral paper as Operate; no Allow/Deny. Not a five-rail WorkBuddy. |
