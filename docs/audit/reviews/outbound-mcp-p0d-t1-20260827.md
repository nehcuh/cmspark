...
| 任务 | CMspark 完成? | Playwright | 谁更合适 |
|------|---------------|------------|----------|
| T1 SSO 邮件前 5 | **Y**（OA门户「我的邮件」widget） | 未跑 | 未对照 |
...
**INCONCLUSIVE（对照臂） / CMspark 臂完成。**

- CMspark：已登录门户上读到「我的邮件」前 5 封，确认台 HITL 生效，禁工具未漏出。
- Playwright 干净 profile **未跑**，不能证伪「Playwright 失败或明显更难」。**禁止**据此扩 outbound profile 或对外声称护城河。
- URL 诚实：操作员写的是 `home.cmschina.com.cn`；实测用的是已打开的 `oa.cmschina.com.cn` OA门户。若 home ≠ 该门户，操作员应否决本完成。
