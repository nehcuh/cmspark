...
| ~~日志轮转~~ | ~~M8~~ | **✅ 已闭环（PR #54）**：kimi 裁决 D1-D4——retention 14d / per-write 大小轮转 10MB 保 1 份 `.1` / 覆盖 mcp/logs(mtime) / `.1.log` 覆盖；新增 `log-rotation.ts`（`pruneOldLogs` 在 `initDataDir` + `rotateLogFileIfNeeded` 在 `logEvent` append 前）；kimi 终审 2 NEEDS-FIX 已修（cutoff UTC 零点消时区 off-by-one + normalize `!== undefined` 让 UI 能 disable）+ pruneByMtime 限 `.log`；+10 测，852 绿 | ✅ |
| ~~LLM 并发+usage+预算~~ | ~~M20~~ | **✅ 已闭环（PR #55）**：kimi 裁决 F1-a/F2-a/F4-0/F5 no-op——纯成本可观测零行为变更。`chatCreate` 加 `stream_options:{include_usage:true}` + 捕获终末 `chunk.usage`，`generateThreadTitle` 捕获 `response.usage`，两处 `logger.info("llm.usage",…)`；F1-a 保持 abort-and-replace（3 abort 点加 `llm.thread_request_superseded`）；F5 今日已实现=no-op；F4 `daily_token_budget` defer P4。+5 测，全量 857 绿 | ✅ 已闭环 |
...
> **进度 (2026-07-13)**：P3 快速清理批次已交付——L1/L3/L4/L5/L9 各取**最小修复**（独立小 PR + kimi 门），L7 经核已在 PR #9 修复（标 close）。剩余 P3 项（god-file 拆分 M12/M14、virtuoso 性能 M15/M16、WS 类型化 M21、杂项 M13/M17、L13/L14 文档版本）defer 独立 PR。
>
> | Finding | 状态 | PR | 说明 |
> |---|---|---|---|
> | L1 原型污染守卫过宽 | ✅ | #57 | 删 value-string 误报检查 + deepMerge 显式 proto-key skip |
> | L3 markdown `<a>` 无 target/rel | ✅ | #60 | DOMPurify afterSanitizeAttributes + http(s)→chrome.tabs.create |
> | L4 死代码 InputArea/ConnectionStatus | ✅ | #58 | 零引用，纯删除（god-file 拆分 M12/M14 defer） |
> | L5 自动滚与用户争抢 | ✅ | #60 | pinnedRef 最小修复（virtuoso M15/M16 defer） |
> | L7 Mermaid pending 竞态 | ✅ | #9 | 早已修（pending+isConnected 守卫齐全），标 close |
> | L9 handleSend 双发 | ✅ | #59 | sendingRef 同步互斥 |
...
- [x] **P0** 紧急止血（P0-1..4）
- [x] **P1** 发布前必修（P1-1..7）→ **可稳定发布 ≥7.0/A**
- [x] **P2** 稳定+隐私+成本（P2-1..4）
- [ ] **P3** 可维护性重构（P3-1..5）— L1/L3/L4/L5/L7/L9 快速清理 ✅（#57-#60）；god-file/virtuoso/WS 类型化/M13/M17/L13/L14 defer
- [ ] **P4** 长期重构（按需）
