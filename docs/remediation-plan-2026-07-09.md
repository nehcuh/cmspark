...
| 任务 | Finding | 改动 | 工时 |
|---|---|---|---|
| unhandledRejection 退出 | M6 | `index.ts:442` 改 exit（对齐 uncaughtException） | 30min |
| 双 shutdown 合并 | M9 | daemon.ts handler await server shutdown promise | 1–2h |
| abort 孤儿消息 | M10 | adapter 每次 addMessage 前查 signal.aborted | 1–2h |
| MCP 子进程 force-kill | M11 | `mcp/client.ts:188` SDK close 后宽限期 kill(pid,SIGKILL) | 1–2h |
