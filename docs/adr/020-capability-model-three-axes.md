...
2. **信任单调**：L2 不得继承 L0 宽松语义；`god-mode` / `auto_approve` **不得**单独静默跳过 CU 任务级 L2（[ADR-017](017-computer-use.md)）。**例外**：[ADR-021](021-unattended-desktop-session.md) 进程内「无人值守」grant（短语+双勾选）可静默 **initial L2 与 mid-task re-L2**（含危险/实验/前台；仅坐标白名单 App；非全局 bool 继承；硬拒绝仍 throw）——属 Trust packaging，不是 Surface 降级。  
