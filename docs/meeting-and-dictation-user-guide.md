...
| **Mtg3.5 声纹** | **自动标说话人（声纹）**：上传音频段 → 本机 ONNX 声纹 embedding + 聚类（需先在 设置 → 听写方式 下载「说话人分离模型」；未就绪会明确提示，**不静默落回旧引擎**）。模型与转写模型共享磁盘预算；音频不出本机。评测门（`companion/scripts/diarize-eval.mjs`）未过显著优于 3 维 baseline 前保持「实验」标注 |
