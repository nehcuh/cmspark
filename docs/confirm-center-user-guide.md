...
**召唤器 HTML 卡（Capture / 「弹出对话框」）上没有 Allow/Deny。** 危险只在本确认台或 Mac 托盘。Win/Linux 没有原生托盘确认，必须打开这扇 Chrome 窗，不能在悬浮卡上点批准。
...
| 在浏览器打开本机文件 | `create_tab` / `navigate`（`file:`） | 解码后的本地路径；**仅这一次**；**不会**加入 MCP 允许目录或域名白名单。系统/凭据/家目录外路径不弹窗、直接拒绝 |
