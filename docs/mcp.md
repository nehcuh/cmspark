...
> CMspark **租手（Outbound MCP）目前是实验能力**：非 default-on、**非产品 ship**（ADR-022）。T1 真人 bake-off（2026-08-27）：CMspark 臂在已登录 OA 完成读邮件 widget；干净 Playwright/Chrome **打不开**同一门户（`ERR_EMPTY_RESPONSE`）。**不是**「看见 SSO 登录页」那种对照；**仍禁止**扩默认 outbound profile。配置成功、工具列表出现 `cmspark__*`，只说明桥通了。记分：[outbound-mcp-p0d-t1-20260827.md](audit/reviews/outbound-mcp-p0d-t1-20260827.md)。
