...
> 版本: 1.8.0 | 日期: 2026-09-20 | 当前阶段：安全稳定化 MVP（核心已完成）→ **产品 0.6.8**。T1 bake-off **已记分**：CMspark 臂完成 / Playwright 干净 profile 打不开门户（L7 PASS 带 nit，[#228](https://github.com/nehcuh/cmspark/issues/228) 已关）。**禁扩**默认 outbound profile。冻 [#230](https://github.com/nehcuh/cmspark/issues/230)（F-S-10 / overlay-acl）。0.5.3 快照：[post-227-status](superpowers/specs/2026-08-27-post-227-status.md)（**SNAPSHOT**）。需求设计必须先开 GitHub Issue。
...
形态 SoT：[2026-08-26-product-form-deepening-design.md](superpowers/specs/2026-08-26-product-form-deepening-design.md)。活切点：[CHANGELOG](../CHANGELOG.md) **0.6.8**。0.5.3 快照：[post-227-status](superpowers/specs/2026-08-27-post-227-status.md)（SNAPSHOT）。
