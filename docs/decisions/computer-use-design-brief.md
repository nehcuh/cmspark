...
+-----------------+                                   | host-use/   <--NEW  |
                                                      ...
                                                      +---------------------+
                                                          ...
                                                          spawn osascript /
                                                          shortcuts CLI /
                                                          screencapture (through companion)
```text
...
**Phase 1 安全保留**（6 个）：`applescript_run`（按 N4 AST 改造）、`app_activate`、`app_list_running`、`ax_tree_query`（blocked_bundles 内）、`screenshot_capture`（按 N15 redact 后）、`file_open_with`（路径白名单）
