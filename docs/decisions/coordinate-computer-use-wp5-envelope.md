...
SEA 臂结论：isSea() 分支 + 旁置 worker eval + createRequire(execPath) 载 ORT 全通，
token 7/7 与非 SEA 一致、延迟差异在噪声内（create 1193 vs 1276ms）——打包形态不引入
性能/正确性回归。O-1 尺寸级围栏：RSS warm ≥800MB 断言真实物化（dummy 通过不算数）。
