...
> **日期**: 2026-08-30  
> **状态**: **r2 LOCKED · kimi+claude 均 AWN**（nits 已折；实现 = 把 `9d45b7c2` r1 铬改写成下列针，不是从零铺）  
...
> **本文件不写代码。** 实现另开 plan + PR，`Closes #256`。r2 dual 已过；plan 节点须再 kimi+claude 一闸。Wave 1 = 把 `9d45b7c2` 改写成 r2。
...
4. **收起状态** = 组件实例 `useState` + ChatView 挂载 `key={activeThreadId}`。换线程/重挂 → 回到该线程的默开/默收。禁 module `Map`、agentStore、sessionStorage、thread 字段。
...
8. **Wave 2 默认 NO-GO。** 开门（全要）：Wave 1 r2 已合；**机核证明 sticky 真坏了**（例如祖先 `overflow` 让收起头滚出视口，而不是「pin-bottom 时头不在视口」——sticky 生效时该条件永假）；live 密度账（StatusRail **48** / Scene **36** / popoutBar **36** / FB 80 / busy+worker 28）附在 PR；Glance 用 FocusBand **已有** 24px secondary，不新开带。过不了 → 本票只收 Wave 1。Sticky 按 r2 工作则 Wave 2 **不必开**——用户选的「后加 B」保留为顺序，不是必须交付。
...
11. **测试**：**改写** `run-progress-ui.test.ts` 里 r1 三断言（`useState(false)` 永远默收、`aria-current="step"`、草稿计入分母），**禁止**在旧锁旁再 append 一套。新锁：纯函数 `defaultExpanded` / `countNM` / `previewText` + 源码锁 `key={activeThreadId}`、sticky 仅收起头或展开 max-height、无 `aria-current="step"`。密度 fixture 钉 live 常数。Companion `run-progress.ts` **零 diff**。
...
13. **`9d45b7c2` 不是 SoT。** 该 commit 已在 `main`（默收、`aria-current="step"`、草稿进 m、展开整卡 sticky 无上限）= r1，四路已打。Wave 1 PR = **把现有铬改写成 r2**，不是从零铺、也不是 dual 未过就当完成。
...
- 源码含 sticky（收起头或展开 max-height 二选一写清）、`aria-expanded`、`key={activeThreadId}`、`prefers-reduced-motion` 或无 transition。
- **删除** r1 断言：`useState(false)` 当作永远默收、`aria-current`+`"step"`、`doneCount}/{total}` 当草稿进分母。
...
| PR | 范围 | Blast |
|----|------|-------|
| **Wave 1** | **改写** `9d45b7c2`：`RunProgress.tsx` + ChatView `key`/sticky + 纯函数 + 改写 r1 测 + DESIGN.md copy 表 | T1 |
