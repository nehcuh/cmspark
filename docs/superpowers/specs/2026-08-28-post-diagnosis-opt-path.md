...
> **状态:** 四路已折 · dual Claude **AWN** + Kimi **AWN**（`both_ok=true`）· 实现须另开 PR，本会话未开工  
> **对抗合成:** [opt-path-adversary-synthesis-2026-08-28.md](../../audit/reviews/opt-path-adversary-synthesis-2026-08-28.md)  
> **体检:** [deep-diagnosis-fanout-2026-08-28.md](../../audit/deep-diagnosis-fanout-2026-08-28.md)  
...
- `companion/src/summoner/overlay-md.ts:32-34`：`href=\""+u+"\"`；`esc` 只 `&<>`。`overlay-md.test.ts:29-36` 无引号突破负例。源码 **没有** `encodeURI`；缺口是引号未转义，不是 encodeURI 用错。
...
- 目录不止五名：`companion/src/bridge/tool-definitions-catalog.json` 含 `list_tabs`/`navigate`/`click`/`get_cookies`/`host_*`/`spawn_worker`/`workspace_*`/`acp_*`/`get_page_html` 等。overlay-eligible `tools.mode=unchanged` 使 whitelist 仍可为 null（`pack-engine.ts:702`）。
...
1. **A1 XSS：** 属性转义（含 `"`）或 `a.href=` DOM 赋值；禁止 `href=""+u`；`javascript:` 仍拒绝。源码没有 `encodeURI`；不要用 encodeURI 当修法。
...
4. **A2 abort 键：** `abortLlmLoopsForPanel(overlay 的 panelId/session)`，**禁止** `abortThreadChat(lease.thread_id)`。不中止 panel 拥有的回路；不把 overlay busy 转交给侧栏。释租约仍要做，否则侧栏永 `OVERLAY_STANDBY`。SSE-only 跳过 pagehide 时也结束 overlay meeting/stt。**IPC：** `handleSummonerClosed` 在 tray（`menu-bar-agent.ts`），`abortLlmLoopsForPanel` 在 companion（`message-router.ts` ~204，由 `ws/lifecycle.ts` 在 WS close 时调用）。hide 不能只在 tray 本地 invalidate：必须经 `companionClient`（或等价 WS）让 companion 对 overlay 那条连接做 abort，再 release leases。**owner stamp：** 今日 `llmLoopOwnerPanel` 只在 `session?.panelId` 有值时写入（~563）。overlay `chat.create` 必须带可寻址 owner（现成 `panelId` 或合成 `overlay:` 前缀），否则 hide 空转。
...
9. **A3 组合绑定：** `pack.apply` / `skill.activate` / `skill.deactivate` / `knowledge.set_active` 的 `thread_id` 必须等于 overlay 租约线程（holder===overlay）。闸打在 **`message-router.ts` 现有 overlay 分支**（`stampedSurface === "summoner"`），不改 `SUMMONER_ALLOW`。允许给 `applySummonerPayloadPolicy` 加「thread 必须是 overlay 租约」而不动 ALLOW 集合；文件地图「acl.ts 本批不改 ALLOW」≠ 整文件不许碰 payload policy。`skill.activate` **不写** `skill_selection_mode`（切片 6）。隐藏 pack/skill 轨仍是 USE-on-current-thread，不是 Operate。
...
12. **B2：** `darwin-estop.ts` + `host.swift` `launchAgentTrayAndExit` + `host-skylight.swift` + `startTrayOwnedEstopBestEffort` 锁步 `--socket-path`。套接字进 `DATA_DIR`（或 0600 + getpeereid / nonce）。预绑 DATA_DIR 套接字也 fail-closed。证明-of-life = 长连 EOF，不是 CONNECT-first。拒绝匿名 `/tmp`。检查 `sun_path` 104。测离开生产 `/tmp`。不改热键/TCC 合同；daemon-spawn 不是打包 happy path。**Skylight 事实：** `--socket-path` 是必填 CLI 参数（缺则 exit 2），**不是**源码写死 `/tmp`；写死 `/tmp` 的是 **flag 文件**默认路径。Node 必须把同一 DATA_DIR socket 传给 Aqua spawn；flag 路径一并离开匿名 `/tmp`。
...
| A1-3 | overlay CSP 不再 `script-src 'unsafe-inline'` **或** 链接只经 `a.href=` 且无 inline handler | grep CSP + 测。`style-src 'unsafe-inline'` 在 DOM 建链分支可留（T3 不靠它挡脚本）；禁止因此把 `script-src 'unsafe-inline'` 一并留在拼接 href 路径上 |
...
- Dual 已齐：Claude AWN + Kimi AWN，nits 已折进本文。**写码另开 PR**（`Refs #245` / `Closes #245`），不在 `main` 上直接实现。
- 实现 agent 不得自评放行；A 批 TDD → 对抗 → dual 才 PR。
