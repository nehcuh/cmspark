...
### 3.1 布局（**v1.1 以 §6 为准** · 浮动纯画布）

```
┌─────────────────────────────────────────────────────────────┐
│ [浮动顶栏] 关联图谱  搜索  孤立点  强度  适应  面板  关闭      │
│                                                             │
│   ○───○          [浮动右栏 可收起]                           │
│  /     \          · 边图例 · 颜色组 · 焦点详情              │
│ ○       ○  ← 全屏力导向画布（近黑）                           │
...
- **画布**：全视口近黑 `#0d0f14`；**无固定侧栏占位**。  
- **Chrome**：浮动顶栏 + 可收起右侧玻璃面板；偏好 `localStorage`（仅显式切换写入）。  
- **空状态**：文案 + 主按钮「为未标注提取要点」→ `thread.extract_digest`（最多 20）；完成后需从侧栏重开图谱刷新快照。
...
|------|------|
| 身份 | 一会话一节点 |
| 标签 | `alias` 或 `未命名 · id前6`；过长 ellipsis；**默认隐藏**，悬停/焦点/放大显示 |
| 尺寸 | 与 **度数** 弱相关（约 2.8–7.5px） |
| 颜色 | **主 tag** 哈希 10 色组（未标注石板灰） |
| 状态 | hover 邻接高亮；focus 柔光；孤立点半透明 |

### 3.3 边

| 类型 | 视觉 | 来源 |
|------|------|------|
| 硬边 | 细实线 | `shared_tags.length ≥ 1` |
| 软边 | 细虚线 / 更淡 | 仅 TF/时间分、无共 tag |
| 粗细 | score 映射，**屏幕像素 ≤ ~1.8** | `buildRelatedEdges` |

### 3.4 手势

| 手势 | 行为 |
|------|------|
| **单击节点** | 展开焦点面板展示详情；**不立即关页** |
...
## 6. 视觉（Obsidian local graph · dark canvas）

- **画布**：近黑 `#0d0f14` 全屏；**无固定侧栏占位**（纯画布）  
- **节点**：小圆点（半径约 2.8–7.5px，随度数弱放大）；**主标签哈希着色**（颜色组，见 `tag-colors.ts`）  
- **焦点 / 悬停**：柔光晕 + 邻接高亮，其余节点/边压暗  
- **边**：细灰线；硬边实线、软边虚线；线宽 ≤ ~1.8 屏幕像素  
- **标签**：**默认隐藏**；仅 **悬停 / 焦点 / 放大 ≥ ~1.15** 显示（9–10.5px 浅灰）  
- **布局**：紧力导向（短弹簧 + 适中斥力）+ 稳定后 fit-to-view  
- **Chrome**：**浮动**顶栏工具条 + 可收起右侧玻璃面板（图例 / 颜色组 / 焦点）；偏好记 `localStorage`  
- **不用** emoji 作节点主图形
...
| **v1.1** | 主 tag 颜色组；标签默认隐藏；浮动纯画布；迷你图例；提取 CTA；a11y 键鼠；snapshot 运行时 slim | landed |
