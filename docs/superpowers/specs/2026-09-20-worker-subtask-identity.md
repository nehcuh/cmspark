...
ThreadList `filtered`、WorkspaceFrame 最近对话、AtThreadPopover 默认池都走同一调词。@ 引用默认排除隐藏的 worker（搜索式 @ 查询仍可命中，标题 `子任务 · {own} · 属于「父」`，无父则 `子任务 · {own}`）。搜索态 ThreadList 不再叠 roleBadge「子任务」。
