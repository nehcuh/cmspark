...
当前阶段：安全稳定化 MVP → 功能扩展中。Side Panel 驱动 Companion/浏览器、线程持久化、tool 调用闭环、安全确认机制已基本完成；正在扩展 Knowledge 系统、Skill Crafting、Daemon 模式、System Tray 等功能。**Obsidian 对话导出已交付**（单条/整 thread 📥 + 🧠 NotebookLM 摘要；vault 档案 + wikilinks/模板，详见 [ADR-008](docs/adr/008-obsidian-export.md)，PR #5）。
